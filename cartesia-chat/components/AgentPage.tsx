...
function ConfigurationTab({ onPreview, previewActive }: { onPreview: () => void; previewActive: boolean }) {
  return (
    <div style={{ maxWidth: 600 }}>
      ...
      {[
        { label: 'System Prompt', content: 'You are a friendly voice assistant built with Cartesia, designed for natural, open-ended conversation…', multiline: true },
        ...
        <div key={field.label} style={{ marginBottom: 20 }}>
          ...
          <textarea
            readOnly defaultValue={field.content} rows={field.label === 'System Prompt' ? 5 : 2}
            style={{ width: '100%', padding: '10px 12px', fontSize: 13, color: '#636260', background: '#fdfdfc', border: '1px solid #dfdcd7', borderRadius: 8, outline: 'none', resize: 'none', cursor: 'not-allowed', boxSizing: 'border-box', fontFamily: 'inherit' }}
          />
        </div>
      ))}
